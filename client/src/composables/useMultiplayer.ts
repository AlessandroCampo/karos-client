...
import { CardPlayedEvent, EventType, GameEvent, GameState, ClientInputRequest, ServerRequest, TargetSelectionRequest, SideEffect, SideEvent } from '@shared/interfaces';
import { myRouter } from '@/router';
import { useAnimator } from './useAnimator';
const { animateCardToBoard, animateAttack, animateCardEffectActivation } = useAnimator();
...
import { getOrCreatePlayerId, sleep } from '@/utils';
...
export const useMultiplayer = () => {
    ...
    onMounted(() => {
        ...
        socket.on('connected', (data) => {
            myPlayerId.value = data.playerId;
            socket.emit('player-auth', { playerId: getOrCreatePlayerId() })
        ...
        socket.on('game-event', async ({ event, state }: { event: GameEvent, state: GameState }) => {
            ...
            if (event.sideEvents) {
                ...
                event.sideEvents.forEach(async sideEv => {
                    if (sideEv.type == SideEvent.EFFECT_TRIGGERED) {
                        await animateCardEffectActivation(sideEv.cardId);
                    }
                })
    ...
    function handleTargetSelection(waitForClient: TargetSelectionRequest) {
        return new Promise((resolve) => {
            ...
            if (waitForClient.playerId == myPlayerId.value) {
                ...
                console.log(availableTargets.value);
